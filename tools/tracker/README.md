...
│   │
│   └── tasks/                      # ⭐ Task definitions storage (consolidated)
│       ├── P{N}-*.json             # Main task objects
│       ├── tests/                  # Separated test suites
│       │   └── P{N}-*-tests.json
│       └── archive/                # Archived task versions
│
...
### 4. data/tasks/*.json ⭐ NEW

**Purpose:** Centralized task definition storage (consolidated from backend/.claude/tasks)

**Structure:**
- **Task Objects:** `data/tasks/P{N}-{FEAT}-T{N}.json` (~300-500 lines each)
  - Task identity, requirements, dependencies
  - References to test suite and workflow
  - Context files and file watch lists
  - Separated architecture: testSuiteRef points to `tests/` directory

- **Test Suites:** `data/tasks/tests/P{N}-{FEAT}-T{N}-tests.json` (~750 tokens)
  - 20-25 comprehensive test cases per task
  - Distribution: 30% unit, 40% integration, 20% edge, 10% e2e/security
  - Separated from task object for token efficiency

- **Archives:** `data/tasks/archive/{timestamp}/` - Versioned task history

**Generated by:** Task Generation Agent (invoked by PM Agent per phase)

**Read by:** Task Execution Agents, PM Agent (for dependency resolution)

**Token Efficiency:** Separated architecture = 93% reduction (task ~250 tokens, tests loaded separately)

**Location Change:** Migrated from `services/backend/.claude/tasks/` to consolidate all tracking data in one location
